...
def run_ai_review(paths: list[Path], timeout: int) -> bool:
    ...
    prompt = f"""Read AGENTS.md and cards/schema.json. Perform a strict second-pass review of only these candidate cards:\n{json.dumps(relative, indent=2)}\n\nCheck technical accuracy, source fidelity, atomicity, English clarity, natural Burmese meaning, safety, duplicate concepts, useful depth, answer correctness, and visual usefulness. Rewrite dense explanations into comprehensible cause-and-effect steps, define unfamiliar terms before using them, and ensure the example tells the learner what to notice. Each language's visual must clarify the core relationship at a glance and remain readable on a phone; reject decorative or confusing diagrams. Correct defects only inside these listed files. Do not inspect Git history or any deleted files. Do not schedule, approve, or change status. Run python scripts/validate_cards.py on these paths. If a claim cannot be supported, set that card's status to rejected. Finish promptly."""
    try:
