...
The dynamic context appears below this template. Create exactly the requested number of cards, one for each listed `(stream, date)` slot, under the category directories in `cards/`. Set each card's `stream` exactly as assigned and use only categories allowed by that stream's configuration. Do not overwrite files. Each card must be approved, atomic, practical, accurate JSON matching `cards/schema.json`, and comfortably under Discord's limit when rendered. Prefer meaningful operational knowledge over trivia. Inspect all existing cards and `state/topics.json` before choosing topics; revisit a topic only for a clearly different advanced skill.
